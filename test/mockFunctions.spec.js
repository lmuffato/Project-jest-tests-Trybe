...
describe('verifica as funções e os mocks', () => {
  ...
  test('testa função factorial', () => {
    const factorialWithReduce = (a) => {
      const array = Array.from(Array(a + 1).keys()).sort((x, y) => y - x);
      array.pop();
      return array.reduce((acc, currentNum) => acc * currentNum);
    };

    mockFunctions.factorial.mockImplementation(factorialWithReduce);
